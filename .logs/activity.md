...
## 2026-07-31 (continued) — Decision: defer Epic 5, then PRD gap audit
Logged to .logs/decisions.md: Epic 5 (real payment gateway, real object storage) deferred indefinitely, staying on the existing MVP mocks -- no code change needed. Pushed (370aa44), CI green.
User then asked to pick up anything else not yet done. docs/stories-wida3-ma.md's backlog (Epics 1-4) is fully complete, but diffing against docs/prd-wida3-ma.md's functional requirements surfaced 3 real gaps never turned into stories: FR-7 (owner has no view of their own listings/status), FR-1 (no edit/deactivate on an existing listing), FR-2 (search doesn't filter by available date range). User chose all three, plus picked "immediate update, no re-review" over "reset to PENDING_APPROVAL on edit" for the edit-approval-flow brainstorm question.

## 2026-07-31 (continued) — PRD gap closure: My Listings, edit/deactivate, availability search
Backend: GET /api/v1/listings/mine (OWNER-only, own listings across all statuses), PUT /api/v1/listings/{id} (owner-only, ownership-checked via 403, immediate update per the chosen no-re-review tier), PATCH /api/v1/listings/{id}/deactivate (reuses the previously-unused ListingStatus.INACTIVE, terminal -- 409 if already INACTIVE, already excluded from search since search is ACTIVE-only). Availability filter added to GET /api/v1/listings/search (availableFrom/availableUntil, NOT EXISTS against overlapping CONFIRMED bookings).
Real bug found and fixed mid-implementation: the availability-filter JPQL initially used `:availableFrom IS NULL OR :availableUntil IS NULL` to toggle the filter, which fails in Postgres/pgjdbc with "could not determine data type of parameter" -- a LocalDate bind parameter whose only appearance is a bare IS NULL check has no type context to infer from. Fixed by passing an explicit `filterByAvailability` boolean parameter instead, so :availableFrom/:availableUntil only ever appear in typed date comparisons.
Frontend: new MyListings.tsx (list + inline edit form + deactivate button) wired into DashboardPage; SearchPage gained "Available from"/"Available until" fields. Caught and fixed a second real bug before it shipped: MyListings' edit form initially reused CreateListingForm's exact field labels ("Title", "City", etc.), which are simultaneously rendered on the same dashboard -- the same duplicate-accessible-name class of bug already fixed once in this project (2026-07-27, SearchPage's "City"/"Location"). Renamed to "Edit title", "Edit city", etc.
Verified: 80/80 backend tests (8 new, including ownership-403 and double-deactivate-409 cases), 89.7% coverage, gate met. Frontend lint/build clean. Extended critical-flows.spec.ts with a new e2e case (create -> see in My Listings -> edit -> deactivate) -- caught and fixed a third bug via this test: the test's own `listingItem` locator (filtered by title text) stopped matching once editing replaced that text with an input value, since Playwright locators re-evaluate live; fixed by scoping edit-form interactions to the section instead of the vanished-text-filtered item. Full e2e suite 4/4 passing locally against live dev servers (backend :8091, frontend :5176, wida3-dev-postgres). Manually verified the date-range filter live via claude-in-chrome (200 OK, correctly filtered results) since it wasn't covered by the e2e spec.
Dev servers and wida3-dev-postgres container stopped at the end of this leg.
